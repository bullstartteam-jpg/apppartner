 ...
 * their …_p01.png twin.
 *   01_PS_C3071-PS_C3090_9_9_AUG17_skin-card_chip_01.pdf
 */
...
export async function buildTiledGangsheet(orders, {
  ...
  // just makes the operator split it by hand. Held as bytes rather than
  // PDFDocument objects so a long chunk does not keep N pdf-lib graphs alive.
  const pagePngBytes = [];
  ...
  return {
    // `blob`/`filename` stay the FIRST page so every existing caller keeps
    // working; `pdfPages` carries the whole set for callers that upload them
    // all, and `baseFilename` is the un-paginated name PNG pages derive from.
